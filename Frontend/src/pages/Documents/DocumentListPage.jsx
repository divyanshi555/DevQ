...
const DocumentListPage = () => {
  ...
  const fetchDocuments=async()=>{
    try {
      ...
      toast.error("Failed to detch documents");
      console.error(error);
    }finally{
      setLoading(false);
    }
    ...
    <div className=' min-h-screen'>
      ...
      {isUploadModalOpen && (<div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm'>
        <div className='relative w-full max-w-lg bg-white/95  backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-slate-900/20 p-8'>
          {/*Close Button */}
          <button className=' absolute top-6 right-6 h-8 w-8 flex items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 transition-all duration-200' 
          ...
          <form onSubmit={handleUpload} className=''>
            ...
            <div className='space-y-2'>
              ...
              <input 
                ...
                required
                className='w-full h-12 px-4 border-slate-200 rounded-xl bg-slate-50/50 text-slate-900 placeholder-slate-400 text-sm font-medium transition-all duration-200 focus:outline-none focus:border-emerald-500 focus:bg-white focus:shadow-lg focus:shadow-emerald-500/10'
                placeholder='e.g Digital Design Notes'
                />
            </div>
            {/*File Upload*/}
            <div className='space-y-2'>
            ...
            <div className='flex gap-3 pt-2'>
              ...
              type="Submit"
              disabled={uploading}
              className='flex-1 h-11 px-4 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98]'
              >
                {uploading ? (
      ...
      {isdeleteModalOpen && (<div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm'>
        <div className='relative w-full max-w-md bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-2xl shadow-slate-900/20 p-8'>
          ...
          <button 
            ...
            className='aboslute top-6 right-6 w-8 h-8 flex items-center justify-center rounded-lg text-slate-100 transition-all duration-200 '
          >
            <X className='w-5 h-5' strokeWidth={2}/>    
          </button>
          {/* Modal Header */}
          ...
          <div className='flex gap-3'>
            ...
            <button
              ...
              disabled={deleting}
              className='flex-1 h-11 px-4 bg-linear-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shaodw-red-500/25 disabled:opacity-50 disabled:cursor-not-allowed active:scale=[0.98]'
            >
              {deleting ?(
                <span className='flex items-center justify-center gap-2'>
                  <div className='w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin'/>
